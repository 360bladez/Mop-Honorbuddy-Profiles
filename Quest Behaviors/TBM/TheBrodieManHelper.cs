...
namespace Styx.Bot.Quest_Behaviors {
    [CustomBehaviorFileName(@"TBMLoader")]
    public class TheBrodieManHelper : CustomForcedBehavior {
        public TheBrodieManHelper(Dictionary<string, string> args)
            : base(args) {
        ...
        #region Dispose
        ~TheBrodieManHelper() { OnFinished(false); }
        ...
        private void LoadNewProfile(string _profile)
        {
            _plugin = _plugin ??
                AppDomain.CurrentDomain.GetAssemblies()
                    .Select(currentassembly => currentassembly.GetType("BrodiesPlugin.BrodiesPlugin", false, false))
                    .FirstOrDefault(t => t != null);
            if (_plugin == null)
            {
                Logging.Write("Brodies plugin is not installed! Cannot load profile: " + _profile);
                return;
            }
